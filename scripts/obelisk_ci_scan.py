#!/usr/bin/env python3
"""OBELISK dependency scanner for CI pipelines and local runners.

This tool discovers dependency manifests in a target repository, resolves
package versions where possible, submits each dependency to OBELISK's
`/api/packages/analyze` endpoint, and enforces risk policy thresholds.
"""
...
def run_scan(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    dependencies, issues = discover_dependencies(repo_path, include_dev=args.include_dev_dependencies)
    ...
    report: dict[str, Any] = {
        ...
        "resolution_issues": [asdict(issue) for issue in issues],
    ...
    if issues and args.fail_on_unresolved:
        report["summary"] = {
            "status": "failed",
    ...
    blocked_count = sum(1 for item in scan_results if item.is_blocked)
    error_count = sum(1 for item in scan_results if item.error)
